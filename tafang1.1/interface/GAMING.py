...
class GAMING():
	def __init__(self, WIDTH=800, HEIGHT=600):
		...
		# 玩家操作用的按钮
		self.buttons = [
							Button(pygame.Rect((info_w+gap), button_y, button_w, button_h), 'fleches', self.takefleches),
							Button(pygame.Rect((info_w+gap*2+button_w), button_y, button_w, button_h), 'arme', self.takearme),
							Button(pygame.Rect((info_w+gap*3+button_w*2), button_y, button_w, button_h), 'canon', self.takecanon),
						...
						# 鼠标点击在工具栏
							...
							for button in self.buttons:
								if button.rect.collidepoint(event.pos):
									if button.text == 'fleches':
										button.onClick()
									elif button.text == 'arme':
										button.onClick()
									elif button.text == 'canon':
										button.onClick()
	...
	def showSelectedInfo(self, screen, button):
		if button.text == 'fleches':
			fleches = Turret.Turret(0)
			selectedInfo1 = self.info_font.render('Cost: '+str(fleches.price), True, white)
			selectedInfo2 = self.info_font.render('Damage: '+str(fleches.arrow.attack_power), True, white)
			selectedInfo3 = self.info_font.render('Affordable: '+str(self.money>=fleches.price), True, white)
			...
			arme = Turret.Turret(1)
			selectedInfo1 = self.info_font.render('Cost: '+str(arme.price), True, white)
			selectedInfo2 = self.info_font.render('Damage: '+str(arme.arrow.attack_power), True, white)
			selectedInfo3 = self.info_font.render('Affordable: '+str(self.money>=arme.price), True, white)
			screen.blit(selectedInfo1, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+35))
			screen.blit(selectedInfo2, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+55))
			screen.blit(selectedInfo3, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+75))
		elif button.text == 'canon':
			canon = Turret.Turret(2)
			selectedInfo1 = self.info_font.render('Cost: '+str(canon.price), True, white)
			selectedInfo2 = self.info_font.render('Damage: '+str(canon.arrow.attack_power), True, white)
			selectedInfo3 = self.info_font.render('Affordable: '+str(self.money>=canon.price), True, white)
			screen.blit(selectedInfo1, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+35))
			screen.blit(selectedInfo2, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+55))
			screen.blit(selectedInfo3, (self.rightinfo_rect.left+5, self.rightinfo_rect.top+75))
		elif button.text == 'XXX':
		...
		if self.money - turret.price >= 0:
			if self.currentMap.get(turret.coord) in self.placeable.keys():
				...
				if self.mouseCarried[1].turret_type == 0:
					...
					self.takecanon()
	'''拿炮塔1'''
	def takefleches(self):
		fleches = Turret.Turret(0)
		if self.money >= fleches.price:
			self.mouseCarried = ['turret', fleches]
	'''拿炮塔2'''
	def takearme(self):
		arme = Turret.Turret(1)
		if self.money >= arme.price:
			self.mouseCarried = ['turret', arme]
	'''拿炮塔3'''
	def takecanon(self):
		canon = Turret.Turret(2)
		if self.money >= canon.price:
			self.mouseCarried = ['turret', canon]
	'''出售炮塔'''
	def takeXXX(self):
